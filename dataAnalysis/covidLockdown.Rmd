...
addLockdownDate <- function(p){
  # assumes p has x = obsDate
  p <- p + annotate("rect", xmin = as.Date("2020-03-26"),
             xmax = as.Date("2020-04-24"), 
             ymin = yMin, ymax = yMax, 
             alpha = localParams$myAlpha, 
             fill = localParams$vLineCol, 
             colour = localParams$vLineCol) 
...
ann_rect <- data.frame(date %between% ("2020-03-26") & as.Date("2020-04-24"),wt = 5,lab = "Text",
                       cyl = factor("Mar",levels = c("Jan","Feb","Mar")))
...
anno <- data.table(x1 = c(as.Date("2020-03-26"), as.Date("2020-04-24")), 
                   y1 = c(0, 8000), 
                   rMonth = c("Mar", "Apr"))
...
yMax <- max(plotDT$PerDiff)
yMin <- min(plotDT$PerDiff)
addLockdownDate(myPlotTot)
