...
```
Figure \@ref(fig:totalgen) shows total generation data for January and February. In addition, a linear model and its error distribution is highlighted in blue. In summary, variation in electricity generation was particularly apparent in February.
...
totalgenDT <- recastDT[, totalMW := Coal+Diesel+Gas+Geo+Hydro+Wind+Wood, keyby =.(rDateTime)]
# Adding a weekday column

# this is easier?
plotDT <- shortDT[, .(totalMW = sum(sumMW)),
                  keyby = .(rDateTime, rMonth)]
...
myPlotTot <- ggplot2::ggplot(plotDT, 
                             aes(x= rDateTime,
                                 ...
                                 colour = rMonth)) +
  geom_line() +
  stat_smooth(method = "gam") + # better than loess
...
In the following we attempt to compare data from January and February. To do this we extract exactly three weeks of data from each month. This is the time interval 06.01.2010-26.01.2020 for Januaray and 03.02.2020-23.02.2020 for February. 
...
totalSelectDT <- totalgenDT[date >="2020-01-06" & 
                            date <= "2020-01-26" |
                            date >= "2020-02-03" &
                            date <= "2020-02-23"]
...
```
This methodology allows us to have the same number of time stamps for each month and is presented in Figure \@ref(fig:threeweekplot). This plot does not show much though. An overlay of the two sets of three week data would be better.
```{r threeweekplot, fig.cap="Three weeks of extracted data for January and February"}

#This shows us the selected weeks for January and February
legend_title <- "Month"
myPlotTot <- ggplot2::ggplot(totalSelectDT, aes(x= rDateTime)) +
  geom_line(aes(y = totalMW, colour = rMonth))+
  scale_color_manual(legend_title, values = c('Jan'='#48C9B0','Feb'='#F4D03F'))+
  theme(axis.text.x = element_text(colour = "black"),
        axis.text.y = element_text(colour ="black"))+
  labs(x='Date', y='Power (MW)') 
  #facet_grid(rMonth ~ .)

yMax <- max(totalSelectDT$totalMW)
yMin <- min(totalSelectDT$totalMW)
addLockdownDateTime(myPlotTot) # adds annotations, needs yMax & yMin to get the box right
```
...
data1 <-data1[, .(Jan = ifelse(totalSelectDT$rMonth == "Jan", totalSelectDT$totalMW, NA ))]
data1 <-data1[!is.na(data1$Jan)]


data2 <-data.table(Feb=numeric())
data2 <-data2[, .(Feb = ifelse(totalSelectDT$rMonth == "Feb", totalSelectDT$totalMW, NA ))]
