...
#ggsave("Total electricity generation.jpeg", dpi = 600, width = 13.333, height = 7.5, units = "in")
```
...
plotDT <- plotDT[, time := hms::as.hms(rDateTime)]
...
```{r coalplot, fig.cap="Total generation for coal by year and month"}
...
```{r TheilSenGWPlot, eval=FALSE, fig.cap=paste0("Theil-Sen plot of mean half-hourly generation per month since ", min(allGridDT$rDateTimeUTC))}

# currently fails as openair needs latticeExtra which needs R (≥ 3.6.0) 
# https://cran.r-project.org/web/packages/latticeExtra/index.html

library(openair)
allGridDT[, rDateTimeNZT := lubridate::as_datetime(rDateTimeOrig)]
allGridDT[, rDateTimeNZT := lubridate::force_tz(tzone = "Pacific/Auckland")] # just in case we're doing this elsewhere

plotDT <- allGridDT[, .(sumMWh = sum(kWh)/1000), keyby = .(rDateTimeNZT)]
plotDT[, date := as.Date(rDateTimeOrig)]  # set date to date for this one
