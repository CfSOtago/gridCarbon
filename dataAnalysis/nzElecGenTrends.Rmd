...
if(local){ # set data storage location
  gcParams$wgenDataLoc <- path.expand("~/Data/NZ_EA_EMI/gridGen/processed/yearly/")
} else {
  gcParams$dataLoc <- path.expand("/Volumes/hum-csafe/Research Projects/GREEN Grid/externalData/")
  gcParams$wgenDataLoc <- paste0(gcParams$dataLoc, "EA_Generation_Data/processed/yearly/")
}
...
```{r generic support, child=gcParams$supportGenericRmd}
```
...
loadData <- function(){
  filesToDateDT <- data.table::as.data.table(list.files(gcParams$wgenDataLoc, ".csv.gz")) # get list of files already downloaded & converted to long form
  
  dt <- do.call(rbind,
                       lapply(filesToDateDT[,V1],
                              function(f)
                                data.table::fread(paste0(gcParams$wgenDataLoc,f))
...
makeColProfilePlot <- function(dt, title){
  myPlot <- ggplot(dt, 
    ...
    geom_col(alpha = 0.4) +
    labs(title = title, caption = gcParams$figCaption, y = "MWh per half hour", x = "Time") +
    theme(legend.title=element_blank(), legend.position="bottom") +
    guides(colour = guide_legend(override.aes = list(alpha = 1))) # resets alpha so we can see the legend
  return(myPlot)
}
