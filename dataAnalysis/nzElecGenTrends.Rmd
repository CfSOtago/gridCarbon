...
if(local){ # set data storage location
  ggParams$lDataLoc <- path.expand("~/Data/NZGreenGrid/safe/ea/")
} else {
  ggParams$lDataLoc <- path.expand("/Volumes/hum-csafe/Research Projects/GREEN Grid/externalData/EA_Generation_Data/")
}

ggParams$genCaption <- paste0("Source: NZ Energy Authority",
                              "\nhttps://www.emi.ea.govt.nz/Wholesale/Datasets/Generation/Generation_MD/")
ggParams$soiCaption <- paste0("Source: The Ministry for the Environment and Statistics New Zealand",
                    "\nhttps://data.mfe.govt.nz/table/89381-monthly-el-nino-southern-oscillation-index-19862016")
ggParams$jointCaption <- paste0(ggParams$genCaption, "\n", ggParams$soiCaption)
```
...
```{r loadDataSample}
filesToDateDT <- data.table::as.data.table(list.files(ggParams$lDataLoc, "long.csv.gz")) # get list of files already downloaded & converted to long form
...
makeMonthlyStackedPlot <- function(dt, yVar, yLabel){
  ...
  ggplot(plotDT, aes(x = as.factor(month), y = get(yVar)/1000000, fill = Fuel_Code)) +
    ...
    facet_grid(. ~ year) +
    labs(caption = ggParams$figCaption, y = eval(yLabel), x = "Month") +
...
makeHalfHourlyColPlot <- function(dt, yVar, yLabel){
  ...
  ggplot(dt, aes(x = rTime, y = get(yVar)/1000000, fill = Fuel_Code)) +
    geom_col(alpha = 0.4) +
    facet_grid(month ~ year) +
    labs(caption = ggParams$figCaption, y = eval(yLabel), x = "Time of Day (local/'civil' time)") +
...
```{r sumProfilePoint2017, fig.height=6, fig.cap="Half-hourly profile plot by fuel type (sum), 2017 only"}
makeHalfHourlyPointPlot(plotDT[year == 2017], "sumkWh", "Sum GWh per half hour across all days of the month")
```
...
makePointProfilePlot <- function(dt, title){
  ...
  ggplot(dt, aes(x = rDateTime, y = sumkWh/1000000, colour = Fuel_Code)) +
    geom_point(alpha=0.4) +
    labs(title = title, caption = ggParams$figCaption, y = "Sum GWh per half hour", x = "Date") +
    theme(legend.title=element_blank(), legend.position="bottom") +
    guides(colour = guide_legend(override.aes = list(alpha = 1))) # resets alpha so we can see the legend
}
...
makePointProfilePlot(plotDT[year == 2017 & month == 6], "June 2017")
makePointProfilePlot(plotDT[year == 2017 & month == 12], "December 2017")
```
...
makeColProfilePlot <- function(dt, title){
  myPlot <- ggplot(dt, 
    ...
    geom_col(alpha = 0.4) +
    labs(title = title, caption = ggParams$figCaption, y = "MWh per half hour", x = "Date") +
    theme(legend.title=element_blank(), legend.position="bottom") +
    guides(colour = guide_legend(override.aes = list(alpha = 1))) # resets alpha so we can see the legend
  return(myPlot)
}
...
makeColProfilePlot(plotDT[year == 2017 & month == 6], "June 2017")
makeColProfilePlot(plotDT[year == 2017 & month == 12], "December 2017")
```
...
soFile <- paste0(ggParams$dataLoc,"externalData/mfe/mfe-monthly-el-nino-southern-oscillation-index-19862016/monthly-el-nino-southern-oscillation-index-19862016.csv")
...
annotateSOIMonths(myPlot)+ # add SOI annotations
  labs(caption = ggParams$jointCaption)
...
ggplot(plotDT, aes(x = year, y = sumkWh/1000000, fill = Fuel_Code)) +
  geom_col() +
  facet_grid(Gen_Code ~ .) +
  labs(caption = ggParams$figCaption, x = "Year", y = "Total GWh per year")
```
...
Should we be adding embedded generation from https://www.emi.ea.govt.nz/Wholesale/Datasets/Metered_data/Embedded_generation ?
