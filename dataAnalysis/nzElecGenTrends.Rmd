 ...
 * Anderson, B. (`r lubridate::year(Sys.Date())`) _NZ Electricity Generation Trends 1998-2020_, University of Otago/University of Southampton: Dunedin.
...
origGenDT <- origGenDT[, rTime := hms::as_hms(rTime)]
origGenDT <- origGenDT[, rDateTimeOrig := rDateTime] # just in case
origGenDT <- origGenDT[, rDateTime := lubridate::as_datetime(rDateTime)]
origGenDT <- origGenDT[, rDateTime := lubridate::force_tz(rDateTime, tzone = "Pacific/Auckland")]
origGenDT <- origGenDT[, rMonth := lubridate::month(rDateTime, label = TRUE, abbr = TRUE)]
...
soFile <- paste0(localParams$dataLoc,"mfe/mfe-monthly-el-nino-southern-oscillation-index-19862016/monthly-el-nino-southern-oscillation-index-19862016.csv")
