...
getModelTrendTable <- function(oa, fname){
  ...
  t <- rDT[,.(peakPeriod, date, conc, nMonths, a,b,slope,expectedVal, diff, pcDiff)]
  return(t)
}

t <- getModelTrendTable(oaGWh, "oaGWh")

ft <- dcast(t[date >= as.Date("2020-01-01")], date ~ peakPeriod, value.var = "pcDiff")
ft[, date := format.Date(date, format = "%b %Y")]
kableExtra::kable(ft, caption = "% above/below expected mean half-hourly GWh by month", digits = 2) %>%
  kable_styling()
...
ft <- dcast(t[date >= as.Date("2020-01-01")], date ~ peakPeriod, value.var = "pcDiff")
ft[, date := format.Date(date, format = "%b %Y")]
kableExtra::kable(ft, caption = "% above/below expected mean half-hourly carbon intensity by month", digits = 2) %>%
  kable_styling()
...
ft <- dcast(t[date >= as.Date("2020-01-01")], date ~ peakPeriod, value.var = "pcDiff")
ft[, date := format.Date(date, format = "%b %Y")]
kableExtra::kable(ft, caption = "% above/below expected mean half-hourly CO2e by month", digits = 2) %>%
  kable_styling()
```
