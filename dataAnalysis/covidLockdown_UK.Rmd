...
```{r descData}
t <- gridGenDT[, .(sC = sum(C02e_T)/1000, # for comparison with https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/875508/final-greenhouse-gas-emissions-tables-2018.xlsx
                   mC = mean(C02e_T), # in T
...
p <- oaGWh$plot
png(filename=here::here("docs/plots/openAir_UK_TheilSenGWhPlot.png"), width = 800, units = "px")
...
```{r calculateCIDiffFromExpected}

t <- getTrendTable(oaCI)

kableExtra::kable(t, caption = "% above/below expected mean half-hourly carbon intensity by month - not June is incomplete", digits = 2) %>%
  kable_styling()
...
```{r calculateCO2eDiffFromExpected}

t <- getTrendTable(oaCO2)

kableExtra::kable(t, caption = "% above/below expected mean half-hourly GHG emissions by month - note June is incomplete", digits = 2) %>%
  kable_styling()
