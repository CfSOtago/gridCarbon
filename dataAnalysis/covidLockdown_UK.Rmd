...
alignedDT[, date := lubridate::as_datetime(paste0(dateFixed, hms))]
openair::timeVariation(alignedDT[dateFixed > as.Date("2020-01-01") & 
                                               dateFixed < today()], 
                       "GW", ylab = "GW", # normalise = TRUE, not useful as divides by lockdown period mean so difference vanishes
                       group = "compareYear", type = "plotPeriodDetailed", key.columns = 2)
```
...
openair::timeVariation(alignedDT[dateFixed > as.Date("2020-01-01") & 
                                               dateFixed < today()], 
                       "GW", ylab = "GW", # normalise = TRUE, not useful as divides by lockdown period mean so difference vanishes
                       group = "compareYear", type = "plotPeriod", key.columns = 2)
```
