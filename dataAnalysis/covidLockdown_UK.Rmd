...
kableExtra::kable(wt, digits = 2,
                  caption = "Monthly total TWh generation by year to June 6th 2020 - note that 2020 is incomplete") %>%
  kable_styling()
```
...
p <- oaGWh$plot
png(filename=here::here("docs/plots/openAir_UK_TheilSenGWhPlot.png"), width = 960, units = "px")
...
Use the openair results object to estimate what the 2020 values would have been if the deaseasoned trend slope had continued. Do this by applying slope.percent to the values for the month 1 year in the past.

> not a good idea: if the Jan 2019 value is an outlier this is not a good estimate

```{r calculateGWhDiffFromExpected}
getTrendTable <- function(oa){
  # oa is an openAir object created by theilSen
  oaData <- as.data.table(oa$data$main.data)
  # if we apply the slope.percent to the 2019 value to estimate what the Jan - > June values should have been
  extract <- oaData[date >= as.Date("2019-01-01") & date <= as.Date("2019-06-01"), 
                    .(date, peakPeriod, conc, slope.percent)]
  extract[date <= as.Date("2020-01-01"), newDate := lubridate::ymd(paste0("2020-", lubridate::month(date), "-01"))]
  extract[, expectedConc := conc + (conc*(slope.percent/100))]
  expected <- extract[, .(newDate, expectedConc, peakPeriod)]
  setkey(expected, newDate, peakPeriod)
  setkey(oaData, date, peakPeriod)
  dt <- oaData[expected]
  dt[, diff := conc - expectedConc]
  dt[, pcDiff := (diff/expectedConc)*100]
  
  tail(dt[,.(peakPeriod, date, conc, slope.percent, expectedConc, diff, pcDiff)])
  ggplot2::ggplot(dt, aes(x = date, y = pcDiff, colour = peakPeriod)) + geom_line()
  
  t <- dt[,.(peakPeriod, date, pcDiff)]
  t <- dcast(t, date ~ peakPeriod, value.var = "pcDiff")
  t[, date := format.Date(date, format = "%b %Y")]
  return(t)
}

t <- getTrendTable(oaGWh)

kableExtra::kable(t, caption = "% above/below expected mean half-hourly GWh by month - not June is incomplete", digits = 2) %>%
  kable_styling()
...
getModelTrendTable <- function(oa){
  # oa is an openAir object created by theilSen
  # oa <- oaGWh
  oaData <- as.data.table(oa$data$main.data)
  oaRes <- as.data.table(oa$data$res2)
  nMonths <- uniqueN(oaData$date)
  jan2020Month <- uniqueN(oaData[date <= as.Date("2020-01-01")]$date)
  feb2020Month <- uniqueN(oaData[date <= as.Date("2020-02-01")]$date)
  mar2020Month <- uniqueN(oaData[date <= as.Date("2020-03-01")]$date)
  apr2020Month <- uniqueN(oaData[date <= as.Date("2020-04-01")]$date)
  may2020Month <- uniqueN(oaData[date <= as.Date("2020-05-01")]$date)
  jun2020Month <- uniqueN(oaData[date <= as.Date("2020-06-01")]$date)
  
  janMt <- data.table::data.table(date = "2020-01-01", 
                                  peakPeriod = oaRes$peakPeriod, 
                                  a = oaRes$a, b=oaRes$b, slope = oaRes$slope, 
                                  nMonths = jan2020Month)
  janMt[, expectedVal := a + (slope*nMonths)] # yes?
  
  febMt <- data.table::data.table(date = "2020-02-01", 
                                  peakPeriod = oaRes$peakPeriod, 
                                  a = oaRes$a, b=oaRes$b, slope = oaRes$slope, 
                                  nMonths = feb2020Month)
  febMt[, expectedVal := a + (slope*nMonths)] # yes?
  
  marMt <- data.table::data.table(date = "2020-03-01", 
                                  peakPeriod = oaRes$peakPeriod, 
                                  a = oaRes$a, b=oaRes$b, slope = oaRes$slope, 
                                  nMonths = mar2020Month)
  marMt[, expectedVal := a + (slope*nMonths)] # yes?
  
  aprMt <- data.table::data.table(date = "2020-04-01", 
                                  peakPeriod = oaRes$peakPeriod, 
                                  a = oaRes$a, b=oaRes$b, slope = oaRes$slope, 
                                  nMonths = apr2020Month)
  aprMt[, expectedVal := a + (slope*nMonths)] # yes?
  
  mayMt <- data.table::data.table(date = "2020-05-01", 
                                  peakPeriod = oaRes$peakPeriod, 
                                  a = oaRes$a, b=oaRes$b, slope = oaRes$slope, 
                                  nMonths = may2020Month)
  mayMt[, expectedVal := a + (slope*nMonths)] # yes?
  
  junMt <- data.table::data.table(date = "2020-06-01", 
                                  peakPeriod = oaRes$peakPeriod, 
                                  a = oaRes$a, b=oaRes$b, slope = oaRes$slope, 
                                  nMonths = jun2020Month)
  junMt[, expectedVal := a + (slope*nMonths)] # yes?
  
  mt <- rbind(janMt, febMt, marMt, aprMt, mayMt, junMt)
  # get the data we want for comparison (only)
  
  extract <- oaData[date >= as.Date("2020-01-01"), 
                    .(date, peakPeriod, conc)]

  setkey(extract, date, peakPeriod)
  mt[, date := as.Date(date)]
  setkey(mt, date, peakPeriod)
  dt <- extract[mt]
  dt[, diff := conc - expectedVal]
  dt[, pcDiff := (diff/expectedVal)*100]
...
t <- getModelTrendTable(oaGWh)

kableExtra::kable(t, caption = "% above/below expected mean half-hourly GWh by month", digits = 2) %>%
  kable_styling()
...
p <- oaCI$plot
png(filename=here::here("docs/plots/openAir_UK_TheilSenCIPlot.png"), width = 800, units = "px")
# dpi = 600, width = 13.333, height = 7.5, units = "in"
plot(p)
dev.off()
...
t <- getModelTrendTable(oaCI)

kableExtra::kable(t, caption = "% above/below expected mean half-hourly carbon intensity by month", digits = 2) %>%
  kable_styling()
...
oaCO2 <- openair::TheilSen(gridGenDT, "C02e_T", 
                  ...
                  type = c("peakPeriod"), layout = c(5,1))
p <- oaCO2$plot
png(filename=here::here("docs/plots/openAir_UK_TheilSenTotalCO2ePlot.png"), width = 800, units = "px")
# dpi = 600, width = 13.333, height = 7.5, units = "in"
plot(p)
dev.off()
...
t <- getModelTrendTable(oaCO2)

kableExtra::kable(t, caption = "% above/below expected mean half-hourly CO2e (T) by month", digits = 2) %>%
  kable_styling()
...
fuelsDT <- gridGenDT[, c("rDateTimeUTC","COAL","GAS","NUCLEAR","WIND","HYDRO","BIOMASS","SOLAR","STORAGE","IMPORTS","OTHER")]
m <- melt(fuelsDT, id.vars = "rDateTimeUTC", variable.name = "Fuel", value.name = "MW")
m[, GW := MW/1000]
...
p <- addLockdownRect(p, 
                ...
                yMin = yMin, yMax = yMax)
p
ggplot2::ggsave(here::here("docs", "plots", "ukGWFuelsTrends.png"), p)
```
