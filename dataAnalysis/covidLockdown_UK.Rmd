...
```{r descData}
t <- gridGenDT[, .(sC = sum(totalC02e_T)/1000000, # for comparison with https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/875508/final-greenhouse-gas-emissions-tables-2018.xlsx
...
sC <- paste0("Total annual CO2e (MtCO2e, calculated)")
maxDate <- paste0("Max date in range")

data.table::setnames(t, c("mGen", "sGen", "mCI", "sC", "maxDate"), # old, must be a better way
                     c(mGen,sGen, mCI,sC,maxDate) # new
...
t[, monthlyTotalC02e_T := sC/nMonths]
...
drake::readd(recentDateTimeGWhPlot)
```
...
drake::readd(recentHalfHourlyProfileGWhPlot)
```
...
drake::readd(compareDailyGWhPlot)
```
...
drake::readd(compareDailyGWhpcPlot)
```
...
oaCO2 <- openair::TheilSen(gridGenDT, "totalC02e_T", 
