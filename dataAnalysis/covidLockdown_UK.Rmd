...
p <- ggplot2::ggplot(plotDT, aes(x = plotDate, y = meanGW, colour = Fuel)) +
  ...
  labs(x = dateLab,
         ...
         caption = "Shaded rectangle: UK lockdown\nTrend : loess")
...
Figure \@ref(fig:makeWeekDayGWhPlot) shows the mean half-hourly CO2e per day. Need to bear in mind the trends identified above.

```{r makeWeekDayGWhPlot, fig.cap="Half-hourly GWh"}
dt <- alignedDT[dateFixed > as.Date("2020-01-01") & 
                                               dateFixed < localParams$toDate]
yVar <- "GWh"
p <- makeWeekdayPlot(dt, yVar = yVar, yLab = yVar, yDiv = 1000)
...
p <- makeWeekdayPlot(dt, yVar = yVar, yLab = "g C02e/kWh", yDiv = 1)
...
dt <- alignedDT[dateFixed > as.Date("2020-01-01") & 
                                               dateFixed < localParams$toDate]
yVar <- "C02e_T"
p <- makeWeekdayPlot(dt, yVar = yVar, yLab = "C02e (kT)", yDiv = 1000)
